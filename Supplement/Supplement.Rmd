...
Let's use a simple neutral model to show that latent traits can be useful for filling in missing links. 

Start by simulated a "true" networks, where attatchment is a neutral process based on species abundance (drawn from a lognormal distribution)

```{r}
#n_plants <- 100; n_frugivores=50; n_links = 500; n_observed = 300; backgroundratio=3 #for debugging
run_latent_trait_simulation <- function(n_plants = 100, n_frugivores = 50, 
                                        n_links = 500, n_observed = 300, backgroundratio=3) {
  ...
  # Degrade the network by hiding some links
  interactions$obs <- NA
  interactions$obs[sample(which(interactions$real == 1), size = n_observed, replace = FALSE)] <- 1 #Correctly get n_observed
  ...
  test <- bind_rows(test, background[sample(nrow(background), backgroundratio * nrow(test)), ]) #Add in some other missed 0's as a background ratio
...
Some plotting investigation of intermediate object

```{r, eval=FALSE}
ggplot(interactions, aes(x = factor(real), y = AbundProd)) +
  geom_boxplot() +
  labs(
    x = "Interaction (0 = Absent, 1 = Present)",
    y = "Product of Interactor Abundances"
  ) +
  theme_minimal()
```


Another option: Instead of relying on entirely neutral processes, let's create an imaginary latent trait that governs interactions. Then we can degrade the network based on abundances and see if we're able to recover that signal strong enough through SVD to get reasonable predictive accuracy. 
```{r}
#n_plants <- 100; n_frugivores=50; n_links = 500; n_observed = 300; backgroundratio=3 #for debugging
run_hidden_trait_simulation <- function(n_plants = 100, n_frugivores = 50, 
                                        n_links = 500, n_observed = 300, backgroundratio=3, exprate=5) {

  # Generate all combinations and assign random abundances AND a random latent feature that controls interactions
  interactions <- expand.grid(FrugivoreID = 1:n_frugivores, 
                              PlantID = 1:n_plants) %>% 
    left_join(data.frame(FrugivoreID = 1:n_frugivores, 
                         FrugAbund = rlnorm(n_frugivores), FrugTrait=runif(n_frugivores, 0,1)), by = "FrugivoreID") %>%
    left_join(data.frame(PlantID = 1:n_plants, 
                         PlantAbund = rlnorm(n_plants), PlantTrait=runif(n_plants,0,1)), by = "PlantID") %>%
    mutate(AbundProd = PlantAbund * FrugAbund, TraitDist=abs(PlantTrait-FrugTrait)) #Make a column of abundance products AND of traitdistance similarity
  interactions$interProb <- dexp(interactions$TraitDist, rate=exprate)/exprate
  
  # Assign "real" interactions NOW USING LATENT FEATURE (interProb)
  interactions$real <- 0
  interactions$real[sample(1:nrow(interactions), size = n_links, replace = FALSE, prob = interactions$interProb)] <- 1
  
which(interactions$real == 1)
  # Degrade the network by hiding some links #NOW MORE LIKELY TO SAMPLE INTERACTIONS BETWEEN ABUNDANT SPECIES
  interactions$obs <- NA
  interactions$obs[sample(which(interactions$real == 1), size = n_observed, replace = FALSE, prob=interactions$AbundProd[which(interactions$real == 1)])] <- 1 #Correctly get n_observed
  interactions$obs[is.na(interactions$obs)] <- 0
  ...
  train <- filter(interactions, !combo %in% test$combo) 
  ...
  out <- woodedWalk_NoSplit(train, 
                            FrugTraits = c("Fsvd1", "Fsvd2", "Fsvd3"), 
                            PlantTraits = c("Psvd1", "Psvd2", "Psvd3"))
...
Let's run these two versions many times and investigate the parameter space. 
```{r}
...
nrep <- 50
result <- NULL
  for(x in seq(0.7,0.99, by=0.01)){ #sampling coverage
    for(y in seq(50,1000, by=50)){ #number of total links
      for(i in 1:nrep){
        temp <- try(data.frame(sampling=x,ntotlinks=y,rep=i,AUC=run_latent_trait_simulation(n_plants = 100, n_frugivores = 50, n_links=y, n_observed=round(y*x))[1]))
        if(class(temp)=="try-error"){
          temp <- data.frame(sampling=x, ntotlinks=y, rep=i,AUC=NA)
        }
        result <- rbind(result,temp)
      }
      print(paste("done with sampling of ", x, " and nlinks of ", y, " with abundance",sep=""))
    }
  }
t1 <- tictoc::toc()
save(t1, result, file="abundanceSummary.rda")
...
nrep <- 50
result <- NULL
  for(x in seq(0.7,0.99, by=0.01)){
    for(y in seq(50,1000, by=50)){
      for(i in 1:nrep){
        temp <- try(data.frame(sampling=x,ntotlinks=y,rep=i,AUC=run_hidden_trait_simulation(n_plants = 100, n_frugivores = 50, n_links=y, n_observed=round(y*x))[1]))
        if(class(temp)=="try-error"){
          temp <- data.frame(sampling=x, ntotlinks=y, rep=i,AUC=NA)
        }
        result <- rbind(result,temp)
      }
      print(paste("done with sampling of ", x, " and nlinks of ", y, " with hidden feature",sep=""))
    }
  }
t2 <- tictoc::toc()
save(t2, result, file="hiddenSummary.rda")

for(i in 1:10){
try(data.frame(sampling=.96,ntotlinks=75,rep=i,AUC=run_latent_trait_simulation(n_plants = 100, n_frugivores = 50, n_links=y, n_observed=round(y*x))[1]))
}
```

Now let's make a plot investigating the outputs of this. 
```{r}
load(file="abundanceSummary.rda")
(t1$toc-t1$tic)/60/60
abundmod <- result
load(file="hiddenSummary.rda")
(t2$toc-t2$tic)/60/60
hiddenmod <- result
```

```{r}
abund_summary <- dplyr::group_by(abundmod, sampling, ntotlinks) %>% dplyr::summarise(., mnAUC=mean(AUC), sdAUC=sd(AUC))
abundplot <- ggplot(abund_summary, aes(x = sampling, y = ntotlinks, fill = mnAUC)) +
  geom_tile(color = "white") +
  scale_fill_viridis_c(name = "Mean AUC", option = "plasma") +
  scale_x_continuous(breaks = seq(0.7, .99, by = 0.01)) +
  scale_y_continuous(breaks = seq(50,1000, by=50)) +
  labs(
    x = "Sampling completeness",
    y = "Number of true links",
    title="abundance-based model"
  )  +
  theme_minimal(base_size = 14) +
  theme(
    panel.grid = element_blank(),
    axis.text.x = element_text(angle = 45, hjust = 0.2, size=8),
    axis.text.y = element_text(size=8)
  )
```

```{r}
hidden_summary <- dplyr::group_by(hiddenmod, sampling, ntotlinks) %>% dplyr::summarise(., mnAUC=mean(AUC), sdAUC=sd(AUC))
    title="hidden feature model"
hiddenplot <- ggplot(hidden_summary, aes(x = sampling, y = ntotlinks, fill = mnAUC)) +
  geom_tile(color = "white") +
  scale_fill_viridis_c(name = "Mean AUC", option = "plasma") +
  scale_x_continuous(breaks = seq(0.7, .99, by = 0.01)) +
  scale_y_continuous(breaks = seq(50,1000, by=50)) +
...
```{r}
#pdf(file="SimulatedPerformance_Latent.pdf")
gridExtra::grid.arrange(abundplot,hiddenplot)
#dev.off()
```
...
nrep <- 50
result <- NULL
  for(x in seq(0.05,0.69, by=0.01)){
    for(y in seq(50,1000, by=50)){
      for(i in 1:nrep){
        temp <- try(data.frame(sampling=x,ntotlinks=y,rep=i,AUC=run_latent_trait_skewsamp_simulation(n_plants = 100, n_frugivores = 50, n_links=y, n_observed=round(y*x))[1]))
        if(class(temp)=="try-error"){
          temp <- data.frame(sampling=x, ntotlinks=y, rep=i,AUC=NA)
        }
        result <- rbind(result,temp)
...
t4 <- tictoc::toc()
save(t4, result, file="abundSkew_smallsamp.rda")
```

```{r}
result2 <- result
load(file="abundSkew_Samplerate.rda")
...
skew_summary <- dplyr::group_by(result, sampling, ntotlinks) %>% dplyr::summarise(., mnAUC=mean(AUC), sdAUC=sd(AUC)) %>% dplyr::filter(., sampling<0.96)

nonskew_summary <- dplyr::group_by(result, sampling, ntotlinks) %>% dplyr::summarise(., mnAUC=mean(AUC), sdAUC=sd(AUC)) %>% dplyr::filter(., sampling<0.96)


ggplot(nonskew_summary, aes(x = sampling, y = ntotlinks/(5000), fill = mnAUC)) +
  geom_tile(color = "white") +
  scale_fill_viridis_c(name = "Mean AUC", option = "plasma") +
...
```
Looking at background points

```{r}
result <- NULL

for(x in seq(0.05,0.95, by=0.01)){ #simulate across sampling completeness values 
    for(y in seq(50,950, by=100)){ # simulate across connectance values
      for(i in 1:nrep){
        temp <- try(data.frame(sampling=x,ntotlinks=y,rep=i,AUC=run_latent_trait_skewsamp_simulation(n_plants = 100, n_frugivores = 50, n_links=y, n_observed=round(y*x), rndm=T)[1]))
        if(class(temp)=="try-error"){
          temp <- data.frame(sampling=x, ntotlinks=y, rep=i,AUC=NA)
        }
        result <- rbind(result,temp)
...
#save(result, file="randomComp.rda")

resutiny <- dplyr::group_by(result, sampling, ntotlinks) %>% dplyr::summarise(., mnAUC=mean(AUC), sdAUC=sd(AUC))

ggplot(resutiny, aes(x = sampling, y = ntotlinks/(5000), fill = mnAUC)) +
  geom_tile(color = "white") +
  scale_fill_viridis_c(name = "Mean AUC", option = "plasma")
...
nrep <- 50
result <- NULL
  for(x in seq(0.05,0.95, by=0.01)){
    for(y in seq(50,1000, by=50)){
      for(i in 1:nrep){
        temp <- try(data.frame(sampling=x,ntotlinks=y,rep=i,AUC=run_latent_trait_skewsamp_simulation(n_plants = 100, n_frugivores = 50, n_links=y, n_observed=round(y*x))[1]))
        if(class(temp)=="try-error"){
...
t5 <- tictoc::toc()
#save(t5, result, file="Neutral_Noabundsamp.rda")

hist(resutiny$mnAUC)
```

Ok, now let's work on the thing of seeing how SVD changes as you degrade a network
```{r}
#n_plants <- 100; n_frugivores=50; n_links = 500; n_observed = 300; backgroundratio=3 #for debugging
run_latent_trait_skewsamp_simulation <- function(n_plants = 100, n_frugivores = 50,
                                        n_links = 500, n_observed = 300, backgroundratio=3, rndm=FALSE, aundweightsamp=T) {
...
Trying again of running across param spaces, this time with variable graph size
