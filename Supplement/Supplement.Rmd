...
interactions <- expand.grid(FrugivoreID = 1:50, PlantID = 1:100) %>% #Make blank pairwise combos
  left_join(., data.frame(FrugivoreID=1:50, FrugAbund=rlnorm(50)), by="FrugivoreID") %>% #Generate random frugivore abundances from lognormal
  left_join(., data.frame(PlantID=1:100, PlantAbund=rlnorm(100)), by="PlantID") %>% #Generate plant abundances
  dplyr::mutate(., AbundProd=PlantAbund*FrugAbund)

interactions$real <- 0
interactions$real[sample(x=1:nrow(interactions), size=500,replace=F, prob=interactions$AbundProd)] <- 1 #Create 500 links, being more likely based on higher abundance
...
interactions
sample(x=1:500, size=400, replace=FALSE)
