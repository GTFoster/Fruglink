...
for(i in 1:2){
  for(j in 1:7){
    temp <- as.data.frame(rf_output[[i]][[j]]$importance)
    temp %<>% rownames_to_column(., var="trait")
    temp$model <- names(rf_output[[i]])[j]
    temp$run <- rf_output[[i]]$run
...
colKey <- vector(mode="character", length=length(levels(varimporSummary$trait)))

colKey[grepl("Phylogeny", levels(varimporSummary$trait))==TRUE] <- Trio_Colors[1]
colKey[grepl("SVD", levels(varimporSummary$trait))==TRUE] <- Trio_Colors[3]
colKey[colKey==""] <- Trio_Colors[2]


#pdf(file="ESA_Figs/VarimporDraft.pdf", width=11, height=11)
...
long <- top2 %>% dplyr::select(., model, combo, scaled_avg) %>% pivot_wider(names_from = model, values_from = c(scaled_avg))
long <- top2 %>% dplyr::select(., combo, Plant_Species, Frugivore_Species, FrugDegree, PlantDegree) %>% unique() %>% left_join(long, ., by = "combo")
long <- long %>% dplyr::select(., -model) %>% unique()

long$Frugivore_Species <- as.factor(long$Frugivore_Species)
long$Plant_Species <- as.factor(long$Plant_Species)
...
comp_raster$y <- as.numeric(long$Plant_Species)
...
myTheme <- BTCTheme()
myTheme$panel.background$col = '#f1ecd3'

pdf(file="Draft_Figs/SuitabilityMap.pdf", width=6, height=6)
levelplot(comp_raster, col.regions=c("white", rev(pal(15))), par.settings = myTheme, layout=c(3, 2), scales=list(tck=c(0,0), draw=FALSE))
dev.off()


levelplot(comp_raster$Trio, col.regions=c("white", rev(pal(15))), par.settings = myTheme, scales=list(tck=c(0,0), draw=FALSE))


#pdf(file="Draft_Figs/SuitabilityMap.pdf", width=6, height=6)
levelplot(comp_raster$Trio, col.regions=c("white", rev(pal(15))), par.settings = myTheme, layout=c(3, 2), scales=list(tck=c(0,0), draw=FALSE))
#dev.off()
...
Eighty5_Sum <- pred %>% dplyr::group_by(., model) %>% dplyr::summarise(., trueP=sum(real == Eighty5 & real ==1), falseP=sum(real < Eighty5), trueN=sum(real == Eighty5 & real == 0), falseN=sum(real!=Eighty5 & real==1))


pred$Fifty <- NA
pred$Fifty[pred$scaled_avg >0.5] <- 1
pred$Fifty[is.na(pred$Fifty)==TRUE] <- 0

Fifty_Sum <- pred %>% dplyr::group_by(., model) %>% dplyr::summarise(., trueP=sum(real == Fifty & real ==1), falseP=sum(real < Fifty), trueN=sum(real == Fifty & real == 0), falseN=sum(real!=Fifty & real==1))

Fifty_Sum$trueP <- as.numeric(Fifty_Sum$trueP)
Fifty_Sum$falseP <- as.numeric(Fifty_Sum$falseP)
Fifty_Sum$trueN <- as.numeric(Fifty_Sum$trueN)
Fifty_Sum$falseN <- as.numeric(Fifty_Sum$falseN)
...
colnames(test) <- c("Phy", "Traits", "Latent","PhyTraits","TraitsLatent", "PhyLatent", "Trio")

for(row in 1:100){
  for(column in 1:7){
    test[row,column] <- split_output[[row]][[column]]$maxJ
  }
}
...
pred$optimal[pred$model=="Latent" & pred$avg > test_sum["Latent"]] <- 1
pred$optimal[pred$model=="Traits" & pred$avg > test_sum["Traits"]] <- 1
pred$optimal[pred$model=="PhyLatent" & pred$avg > test_sum["PhyLatent"]] <- 1
pred$optimal[pred$model=="PhyTraits" & pred$avg > test_sum["PhyTraits"]] <- 1
pred$optimal[pred$model=="TraitsLatent" & pred$avg > test_sum["TraitsLatent"]] <- 1
pred$optimal[pred$model=="Trio" & pred$avg > test_sum["Trio"]] <- 1
...
confusion <- compTable %>%
  ...
  column_spec(2, color = spec_color(OptimalThresh_Prop$trueP, end=0.8)) %>%
  column_spec(4, color = spec_color(OptimalThresh_Prop$trueN, end=0.8)) %>%
  column_spec(3, color = spec_color(OptimalThresh_Prop$falseP, direction = -1, end=0.8))%>%
  column_spec(5, color = spec_color(OptimalThresh_Prop$falseN, direction = -1, end=0.8))%>%
