...
```{r}
syns <- c("Aburria jacutinga", "Baillonius bailloni", "Myiothlypis flaveola", "Setophaga pitiayumi")
tol <- taxize::tol_resolve(syns)
tol$unique_name %in% gsub("_", " ", Bird_trees[[1]]$tip.label)

birdlist<-c(unique(birds$Frugivore_Species), tol$unique_name, "Aburria aburri", "Setophaga ruticilla") #Including two species with only congenerics in ds missing from Jetz tree.

#library(clipr)
#clipr::write_clip(birdlist)
```
...
split_output <- list()
for(i in 1:100){
    nas <- dplyr::filter(birds_temp, is.na(fc1)==TRUE) %>% dplyr::select(., Frugivore_Species) %>% unique() #Bird taxa that don't occur in our tree
    ...
    for(i in 1:nrow(nas)){ #add in our missing genera as polytomies
      polytree <- phytools::add.species.to.genus(tree=polytree, species=nas$Frugivore_Species[i], where="root") #Create polytomy at the genera root. 
    }
