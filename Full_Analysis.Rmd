...
Since we have only one phylogeny for plants, we do this once and assign the resultant eigenvectors as columns in our dataset. This isn't currently run, since we've amended the code to first include more species as polytomies when congenerics are available. 
```{r}
...
Bien Phylogeny contains 646 out of 787 total plants. By including species with congenerics into our tree as randomly assigned polytomies within the genus, we can add an additional 46 species. 
```{r}
...
Here we recompute the phylogenetic decomposition, including the congeneric polytomies. 
```{r}
...
## 3. Run Randomforest on Test-train split w/Replicates
Run 100 times! By default this is set to eval=FALSE in order to reduce overall runtime, but to run it just change to TRUE. (The output object is included below)
```{r, eval=False}
split_output <- list()
for(i in 1:100){
...
Loading in the results of the above analyses #CHANGE BACK TO TRUE
```{r, eval=FALSE}
...
Trio_Colors <- c("#332288ff", "#44aa99ff", "#11E0E4") #Phylo, Trait, Latent
```
...
#pdf(file="Figures/VarImportance_ErrBars_Short_col2.pdf", width=7, height=3.5)
ggplot(data=varimporSummary, aes(y=trait2, x=avg, fill=model))+geom_col(position = position_dodge(), width = 0.9)+theme_classic()+
  scale_fill_manual(values = alpha(c("Phy" = "#332288ff",
                                "Traits" = "#44aa99ff",
                                "Latent" = "#88cceeff",
                                "PhyTraits" = "#117733ff",
                                "TraitsLatent" = "#aa4499ff",
                                "PhyLatent" = "#cc6677ff",
                                "Trio" = "#ddcc77ff"
                                ), 1.0))+
...
Including a bit of exploration of our top models. 
```{r}
top <- dplyr::filter(x, Trio>quantile(x$Trio, .9995, na.rm=T))
topbirds <- unique(sub("-.*", "", top$combo))

median(table(birds$Frugivore_Species))
table(birds$Frugivore_Species)[names(table(birds$Frugivore_Species)) %in% topbirds]
median(table(birds$Frugivore_Species)[names(table(birds$Frugivore_Species)) %in% topbirds])
