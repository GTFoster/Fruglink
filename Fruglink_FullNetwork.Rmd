...
```{r}
Phy <- woodedWalk_NoSplit(birds, FrugTraits = c(f_phylo), PlantTraits = c(p_phylo), class_balancing = FALSE)
colnames(Phy) <- c("Frugivore_Species", "Plant_Species", "real", "Phy")

Traits <- woodedWalk_NoSplit(birds, FrugTraits = c(f_traits), PlantTraits = c(p_traits), class_balancing = FALSE)
colnames(Traits) <- c("Frugivore_Species", "Plant_Species", "real", "Traits")

Latent <- woodedWalk_NoSplit(birds, FrugTraits = c(f_latent), PlantTraits = c(p_latent), class_balancing = FALSE)
colnames(Latent) <- c("Frugivore_Species", "Plant_Species", "real", "Latent")


PhyTraits <- woodedWalk_NoSplit(birds, FrugTraits = c(f_phylo, f_traits), PlantTraits = c(p_phylo, p_traits), class_balancing = FALSE)
colnames(PhyTraits) <- c("Frugivore_Species", "Plant_Species", "real", "PhyTraits")

TraitsLatent <- woodedWalk_NoSplit(birds, FrugTraits = c(f_latent, f_traits), PlantTraits = c(p_latent, p_traits), class_balancing = FALSE)
colnames(TraitsLatent) <- c("Frugivore_Species", "Plant_Species", "real", "TraitsLatent")


PhyLatent <- woodedWalk_NoSplit(birds, FrugTraits = c(f_latent, f_phylo), PlantTraits = c(p_latent, p_phylo), class_balancing = FALSE)
colnames(PhyLatent) <- c("Frugivore_Species", "Plant_Species", "real", "PhyLatent")


Trio <- woodedWalk_NoSplit(birds, FrugTraits = c(f_phylo, f_latent, f_traits), PlantTraits = c(p_phylo, p_latent, p_traits), class_balancing = FALSE)
...
```{r}
spear.matrix <- cor(composite[,4:ncol(composite)], method="spearman")
corrplot::corrplot(spear.matrix, type = "lower", diag=T, order = "AOE")

corr.matrix <- cor(composite[,4:ncol(composite)], method="pearson")
corrplot::corrplot(corr.matrix, type = "lower", diag=T, order = "AOE")
...
heatmap(as.matrix(selection_0), Rowv = NA, Colv=NA)
```
